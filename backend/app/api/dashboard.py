...
async def by_status(db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    return await dashboard_service.get_by_status(db)


@router.get("/by-priority")
async def by_priority(db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    return await dashboard_service.get_by_priority(db)


@router.get("/by-sector")
async def by_sector(db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    return await dashboard_service.get_by_sector(db)


@router.get("/by-technician")
async def by_technician(db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
    return await dashboard_service.get_by_technician(db)


@router.get("/sla-compliance")
async def sla_compliance(db: AsyncSession = Depends(get_db), admin: User = Depends(require_admin)):
